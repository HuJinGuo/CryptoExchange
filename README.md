...
## 项目概述
CryptoExchange 是一个基于 Go 语言的加密货币交易所后端系统，专注于 ETH/USDT 交易对的撮合引擎和流动性管理。项目源于 Anthony 的教学仓库（anthonygg/cryptoExchange），用于模拟真实交易所环境，包括自动做市商机器人和链上结算。仓库地址：https://github.com/HuJinGuo/CryptoExchange。当前为开发/测试阶段，适合学习 Web3 开发和量化交易。
...
| 优先级 | 功能模块 | 具体改进点 |
|------|----------|----------|
| ★★★★★ | 核心安全 | 1. 加入用户余额系统（ETH 余额表）<br>2. 下单前检查余额 + 挂单锁仓<br>3. 市价单禁止超额成交 |
| ★★★★★ | 稳定性 | 4. 彻底移除 `panic("not enough volume")`，全部改为返回错误<br>5. 做市商每次循环前撤掉自己所有旧单 |
| ★★★★ | 链上交互 | 6. 链上转账金额单位修复（×1e18）<br>7. 可开关真实链上转账（开发时关闭）<br>8. 增加交易手续费（0.05%-0.2%） |
| ★★★★ | 做市商增强 | 9. 做市商支持多档挂单（3~5 档深度）<br>10. 动态调整 offset（波动率越大，价差越大）<br>11. 支持从多个交易所拉价格（Coinbase、OKX） |
| ★★★★ | 多交易对 | 12. 支持 BTC/USDT、SOL/USDT 等<br>13. Market 枚举化，订单簿 map[Market] |
| ★★★ | 用户系统 | 14. 注册/登录 API（JWT）<br>15. API Key 认证<br>16. 私有订单查询 |
| ★★★ | 持久化 | 17. 用 Redis/PostgreSQL 存储订单、余额、交易历史<br>18. 重启后恢复订单簿 |
| ★★★ | WebSocket | 19. 实时推送深度、成交、用户订单状态 | 
| ★★ | 前端 | 20. React/Vue 简易交易界面（盘口、下单、K 线） | 
| ★★ | 风控 | 21. 单用户频率限制<br>22. 最大单笔限制<br>23. 异常撤单监控 | 
| ★★ | 监控告警 | 24. Prometheus + Grafana 监控<br>25. 盘口深度、价差、gas 异常告警 |
| ★ | 高级功能 | 26. 支持 Maker 费率优惠<br>27. 止损/止盈单<br>28. 资金费率（永续合约） |
